...
This file describes how you should read, edit and extend the HOI4 codebase.
...
Before you open or edit any HOI4 file, you must consult the relevant Hearts of Iron IV modding pages from the offline Paradox wiki snapshot in `paradox_wiki/`.
...
- Vanilla Hearts of Iron IV includes official documentation files (often in markdown).
  - The folder `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\documentation` contains markdown documentation files that **must be read**.
...
If HOI4 already has a pattern for the same thing, follow that over vanilla for consistency, but still take a look at a vanilla implementation.
...
- Kaiserreich (1521695605) is approved as a reference mod for structure, patterns, and edge case handling. The local Hearts of Iron IV launcher workspace is `C:\Users\dev-cop\Documents\Paradox Interactive\Hearts of Iron IV`. `2265420196` and `1458561226` are also approved as reference mods.
...
- Use `the events skill` for HOI4 event implementation, event logs, evolutions, event details, documentation, and spreadsheet alignment.
- Use `the event asset skill` when an event needs visual assets, icons, flags, portraits, UI art, report images, news images, achievement icons, final DDS files, asset manifests, or sprite handoff notes.
- Use `the frame animation skill` when a task needs animated sprites, frame sequences, sprite sheets, GIF previews, animated UI pieces, animated portraits, hover loops, pulse loops, route emblems, or frame-by-frame visual packages. This skill forbids final animation made only by moving, scaling, rotating, warping, blurring, recoloring, or filtering one still image.
- Use `the super-events skill` when a task creates, updates, researches, or wires a super-event.
...
- Use `the subagent skill` when coordinating custom Codex subagents, routing bounded work, or defining parent/subagent ownership boundaries.
- Use `the improvement loop skill` when an implemented or planned mechanic needs recursive depth expansion, spec addenda, improvement handoffs, or checks for shallow, duplicated, generic, disconnected, or low-impact content.
...
`the subagent skill` is the detailed source of truth for subagent routing, ownership boundaries, handoff quality, audit cadence, asset routing, super-event routing, and the recursive mechanic expansion loop.
...
- When an asset subagent is asked to produce animation, the parent prompt must require `the frame animation skill` and must ask for separate generated, sourced, or provided source frames, static fallback, manifest, contact sheet, preview, and `.gfx` or `.gui` handoff.
...
9. Constants `@MY_CONSTANT` cannot cross file boundaries. They are file scoped.
   ...
   - Required vanilla docs:
     - `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\documentation/script_concept_documentation.md` (Script Constants section)
     - `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\common/script_constants/documentation.md` (schema + examples)
   - Where to put them:
...
10. Use event targets (`event_target:`) to persist a scope pointer across blocks/events when variables/scopes alone are insufficient.
    ...
    - `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\documentation/effects_documentation.md` (`save_event_target_as`, `save_global_event_target_as`, `clear_global_event_target`, `clear_global_event_targets`)
    - `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\documentation/triggers_documentation.md` (`has_event_target`)
...
Add prefixes if a folder is dedicated to HOI4 files that all share the prefix. Do not add the `chaosx` prefix to variable names, scripted effects, or scripted triggers unless the surrounding context uses it consistently.
...
2. In addition to the Paradox wiki, inspect vanilla files in `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\` and read all the necessary documentation, particularly in `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\documentation`.
...
13. Respect the repository style and naming rules so new content blends with existing HOI4 code.
...
20. Respect the player-facing writing style found in `the events skill`.
...
For HOI4 event implementation, use the repo skill `the events skill`.
...
Before claiming focus-tree completion, use the appropriate audit route from `the subagent skill`. If a tree works but feels shallow, duplicated, generic, or disconnected from gameplay, use `the improvement loop skill` and consider a plan-mode pass from `chaosx_improvement_loop_planner`.

## 8. Agent-generated Visual Assets

For final visual assets, use `the event asset skill`. That skill is the detailed source of truth for image generation rules.

For animated visual assets, use `the frame animation skill` in addition to `the event asset skill`. Final animation assets must be built from real planned frames and must not be transform-only mockups.

Use `the subagent skill` for detailed asset subagent routing. Asset subagents create source files, processed PNGs, DDS files, manifests, and handoff notes. The main agent owns `.gfx` edits, gameplay references, localisation references, documentation alignment, and final validation.
