---
name: the frame animation skill
description: Use when HOI4 needs animated sprites, animated icons, animated scripted GUI pieces, animated portraits, GIF previews, frame sheets, hover loops, pulsing glows, floating seals, warning pulses, route emblems, or frame-by-frame asset packages. Requires real per-frame source artwork or source frames. For HOI4, the final in-game animation must be a frame sheet texture wired through the verified `.gfx` and `.gui` pattern, usually `frameAnimatedSpriteType`, not a saved GIF.
---

## 1. Core rule

A final HOI4 animation is a HOI4 frame-sheet asset package.
...
Do not use primitive shape animation as final art. Circles, rectangles, lines, gradients, simple icons, oscilloscope-style waves, bars, and other geometry-only frames are acceptable only as explicitly labeled planning diagrams or temporary debug previews, never as final HOI4 artwork.
...
Do not assume every HOI4 surface accepts animated sprites in the same way. Before wiring, inspect the offline Paradox wiki snapshot, vanilla examples, local documentation, and existing HOI4 examples for the exact target surface.

## 3. Relationship with other skills

Use this skill with `the event asset skill`.

`the event asset skill` owns source mode, reference folders, DDS conversion, manifests, final asset placement, and sprite handoffs. This skill owns frame planning, per-frame generation discipline, anchor normalization, sheet construction, GIF previews, contact sheets, and HOI4 animation QA.
...
Use `the super-events skill`, `hoi4-focus-trees`, `hoi4-decisions-missions`, or `the events skill` for the gameplay surface that uses the animation.
...
- relevant vanilla `.gfx` and `.gui` files under `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\`
- existing HOI4 `.gfx`, `.gui`, portrait, focus, decision, and scripted GUI examples when available
...
If the surface has no clear vanilla or HOI4 animated precedent, report a blocker or ask for permission to run a narrow exploration pass.
...
The handoff must include a ready-to-copy snippet, but the implementation agent must verify it against local vanilla and HOI4 examples before using it.
...
- local wiki page, vanilla file, or HOI4 example used as wiring precedent
...
1. Inspect the actual vanilla GUI surface that displays the portrait. Common country-leader surfaces include:
   - `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\interface/countrypoliticsview.gui` for the current country's politics leader portrait.
   - `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\interface/countrydiplomacyview.gui` for a selected country's diplomacy leader portrait.
2. Record the exact vanilla element coordinates, scale, and parent containers used to reach that element.
3. Create a separate independent overlay container in a HOI4 `.gui` file. Do not edit vanilla GUI files for the overlay unless the task explicitly requires a vanilla GUI override.
...
- the offline HOI4 wiki page, vanilla file, or existing HOI4 example needed for wiring cannot be inspected
