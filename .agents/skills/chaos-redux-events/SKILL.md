---
name: the events skill
description: Use when implementing or updating HOI4 events.
---

# HOI4 Events

Use this skill for any HOI4 event work, including:
...
Repository-wide reading and style rules live in `AGENTS.md`. This skill only adds the HOI4 event-specific implementation contract.
...
In HOI4, an event is not just an event block.
...
- Spawn `chaosx_asset_source_researcher`, `chaosx_generated_event_art`, and `chaosx_icon_artist` for actual visual asset packages according to `the event asset skill`.
- Spawn `chaosx_super_event_text_researcher` and `chaosx_super_event_audio_researcher` for actual super-event research packages according to `the super-events skill`.
...
These writing style rules apply to every HOI4 prose surface, including event text, news text, super-event text, decision text, focus descriptions, tooltips, docs, specs, plans, prompts, spreadsheets, and all player-facing text.
...
For super-events, do not invent quotes, cultural remarks, song fragments, title references, or final audio choices. Use `the super-events skill` and the relevant research subagents when the event needs sourced wording or music.
...
Triggerable scenarios are manual sandbox or challenge setups launched from the HOI4 settings UI. They are separate from the normal random-event timer, chaos-tier eligibility, evolution pacing, and automatic source-event prerequisites.
...
9. Document the scenario in the relevant event doc or scenario doc, using the vanilla documentation tree under `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\documentation` as the reference base.
...
- `common/achievements/hoi4_achievements.txt` for the single root-only HOI4 achievement registry. Keep event achievements grouped by event section inside this file instead of creating per-event achievement files with new `unique_id`s
...
Events must appear in HOI4’s event log, wire the full log contract in the same change.
...
Use `the super-events skill` whenever an event includes a super-event.
...
- `docs/spreadsheets/events_catalog.xlsx`
...
Use the `the event asset skill` skill whenever an event task requires generated or processed visual assets.

For every generated asset:

1. Create the base artwork by following the official `$imagegen` skill workflow through `the event asset skill`.
...
9. If the event has a super-event, `the super-events skill` has been used for quote, remark, audio, and presentation planning.
10. Supporting decisions, ideas, AI, country setup, or exclusions are updated if relevant.
11. `docs/events/` is updated.
12. `docs/spreadsheets/events_catalog.xlsx` is updated.
13. If assets are required, `the event asset skill` has been used.
