---
name: the super-events skill
description: Use when designing, researching, wiring, or documenting HOI4 super-events.
---

# HOI4 Super Events

Use this skill when a HOI4 task creates, updates, researches, or wires a super-event.
...
General event implementation belongs to `the events skill`.

Visual asset sourcing and processing belongs to `the event asset skill`.

## 1. Core purpose

A HOI4 super-event is a major presentation moment.
...
- super-event image handoff to `the event asset skill`
...
Use `the events skill` for the full implementation contract:
...
### `the event asset skill`

Use `the event asset skill` for:
...
Super-event localisation must follow the existing HOI4 format.
...
Use the existing HOI4 settings-aware playback helper. Do not bypass it.
...
When implementing audio, follow the existing HOI4 audio pattern.
...
Super-event images are handled through `the event asset skill`.
...
- follow `the event asset skill` and the official `$imagegen` workflow for generated images
...
- guard the branch with the world-end rules from `the events skill`
...
9. The image direction has been handed to `the event asset skill`.
