...
- `the events skill` when the tree belongs to an event
- `hoi4-decisions-missions` when focuses unlock, modify, or depend on decisions and missions
- `the event asset skill` when focus icons, leader portraits, flags, or idea icons are required
...
- Read relevant vanilla documentation from `D:\SteamLibrary\steamapps\common\Hearts of Iron IV\\documentation`.
- Inspect vanilla focus files for syntax and layout precedent.
- Inspect existing HOI4 focus trees and event-created focus-tree loading patterns.
- Read `AGENTS.md`.
