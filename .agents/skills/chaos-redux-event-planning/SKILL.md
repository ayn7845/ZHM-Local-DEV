---
name: the event-planning skill
description: Use when expanding HOI4 event ideas into detailed specifications before implementation.
---

# HOI4 Event Planning

Use this skill to design or expand events for the Hearts of Iron IV mod HOI4.

This skill creates event specifications directly inside the HOI4 repository. It does not implement code. Implementation belongs to `the events skill`. Visual asset generation and processing belongs to `the event asset skill`. Animated sprite planning, frame-sheet requirements, animated portrait packages, and animation handoff details belong to `the frame animation skill` when motion is needed. Super-event quote, remark, music, and presentation research belongs to `the super-events skill`.
...
- `AGENTS.md`
- `the events skill`
- `the event asset skill` when the event needs visual assets
- `the frame animation skill` when the event has animated sprites, animated UI, animated route emblems, animated portraits, warning pulses, hover loops, glow loops, float loops, particle loops, or frame-by-frame presentation needs
- `the super-events skill` when the event needs a super-event
...
- provided HOI4 mechanics docs

Use those sources to understand how HOI4 already works, then expand creatively from there.
...
The goal is to create an event that feels like a layered HOI4 system, not a basic popup.
...
- interactions with existing HOI4 systems
- documentation needs
...
Achievement design must include asset planning. Each achievement needs a 64x64 completed icon direction, and the asset brief must hand those icons to `the event asset skill`. Grey, locked, and not-eligible variants can be produced later if the achievement system requires them.
...
- crisis pressure, threat-meter components, condemnation, deaths, pollution, contamination, or other HOI4 system values when relevant
...
- documentation needs
- compatibility notes if the country already exists in vanilla, HOI4, or common mods
...
- what other HOI4 systems it should interact with
...
## 5. HOI4 system awareness

Consider links to existing HOI4 systems when they strengthen the event.
...
For each planned animated asset, the spec should define the in-game use, target surface, state logic, frame count expectation, loop behavior, static fallback, source mode, asset handoff owner, and proposed sprite names when they are known. The final animation must follow `the frame animation skill`, meaning real source frames, a frame sheet, a static fallback, a preview GIF for review only, and a `.gfx` handoff. Do not describe a GIF, filter pulse, recolour loop, shifted still image, or transform-only mockup as the final game animation.
...
Do not fully research quotes, cultural remarks, or music inside this skill. Use `the super-events skill` for that work.

The event spec should provide enough direction for `the super-events skill` to find real quotes, meaningful cultural remarks, and suitable audio.
...
Important super-event boundary: this planning skill may define super-event role, trigger, tone, image direction, quote direction, cultural-remark direction, and audio mood. Any source-dependent wording belongs to `the super-events skill` and must stay blocked until researched and documented.
...
Do not create user-facing continuation text blocks as deliverables. This skill is for coding agents working directly in the HOI4 repository. The agent should continue by writing the next spec file, not by asking the user to paste text into a later chat turn.
...
Asset generation, sourcing, cropping, resizing, DDS conversion, file placement, sprite handoff notes, and manifests belong to `the event asset skill`. Animated frame planning and frame-sheet handoff requirements belong to `the frame animation skill`. Final `.gfx` wiring belongs to the main implementation agent unless a parent prompt explicitly grants that scope.
...
Use the installed HOI4 tree under `D:\SteamLibrary\steamapps\common\Hearts of Iron IV` as the reference baseline for vanilla examples.

When the event spec calls for asset examples, the asset brief should say which vanilla folder or repository asset folder to inspect instead of naming a project-internal placeholder reference path.
## 14. Asset brief handoff

After the full event specification is complete, create a separate asset brief file for `the event asset skill`.
...
Use `the event asset skill` rules for source selection. Symbolic icons usually use `$imagegen`. News event images, report event images, and super-event images may be sourced or generated. prefer generated assets for fictional, alternate-history, symbolic, high-chaos, or unique scenes, and sourced assets for real historical people, real photographed events, and real archival artifacts. Historical flags and historically attested symbols should be sourced and documented, then converted to HOI4 flag sizes. Fictional, supernatural, invented, or alternate-history flags can use `$imagegen` through `the event asset skill` when appropriate.
...
If the event has one or more super-events, create a separate super-event research brief file for `the super-events skill`.
...
The `the super-events skill` research brief should ask the agent to:
...
- coordinate super-event image needs with `the event asset skill`
...
When `the improvement loop skill` produces an expansion addendum, treat it as event-planning input. The addendum should be folded into the main spec pack with the same seriousness as the original user idea. Do not treat it as a loose suggestion if the parent accepted it.
...
This skill is for coding agents working directly in the HOI4 repository. Do not create a zip package, external bundle, or user-facing collection of loose files.
...
Create an asset brief for `the event asset skill` when the event needs visual assets.
...
Create a super-event research brief for `the super-events skill` if the event has one or more super-events.
...
- follow `AGENTS.md`
- follow `the events skill`
- use `the event asset skill` if visual assets are required
- use `the super-events skill` if super-events are required
...
- animated sprite and animated portrait needs mapped with static fallbacks, state logic, and `the frame animation skill` handoff expectations when relevant
...
- animated asset plan that lacks static fallback, state logic, frame-sheet handoff, target surface, sprite names, or `the frame animation skill` ownership
